...
function reqOptions (options) { // 上载做好的图片资源
    ...
    // if(_isStream){
    //     const messageS = await readStream (_path, reqOptions)  // 返回失败还是成功的消息
    //
    // }else{
    //     const message = await readFile (_path, reqOptions)
    // }
...
/* 对失败的内容自动重新发送一次解决思路：几种️方法
    1. 通过定时器去检查 pass
    2 通过多进程，利用负载均衡，对失败的内容重新fork子进程去处理，利用主节点的调度 yes
